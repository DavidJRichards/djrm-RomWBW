...
#ENDIF
;
CLCD_DAT	.EQU	CLCD_BASE + 0		; DATA PORT
...
;
#ifndef  CLCDTEST
;T6963_INIT; *** TODO: CGA AND EGA ARE PLACEHOLDERS.  THESE EQUATES SHOULD
...
; 
;	      	Display Size: 40 Column × 16 Lines 
; 
;	      	Character Font: 6 or 8 Dots Mode hardware selectable
; 
...
; 
		...
		CALL CMD 
; three lots of 128 bytes - todo make one long loop
...
; todo allow for bit set and clear for combining masks		
...
; todo: wind this up and make function calls for text and attribute scrolling with addrerss passing
#if 1
; scroll attributes 
...
; scroll text 
#if 0
                ld      hl,SCROLL_TEXT             ; top of screen
                ld      (SCROLLER),hl

scrollmore:
                push     hl                        ; save this line
				
                ; add 040H to HL (smashes bc)
                ld      a, 040H                    ; bump to next
                ld      c,a
                ld      b,0
                add     hl,bc
                ld      (SCROLLER),hl              ; save next line
                
                ld      bc,0400H                   ; end of screen
                sbc     hl,bc                      ; compare to new addr
 
                pop     HL                         ; retrieve unbumped version
                jr      nc,scrollend               ; all done

		call    READBUFFER                 ; addr in hl                
                ld      hl,(SCROLLER)              ; write next line
		call    WRITEBUFFER                
...
; splash message with attributes in box - used for display sanity startup check       
DJRM1:
...
TXBUF2:
		.DB	"Now is the time for all good me to come_to the aid of the party." 
